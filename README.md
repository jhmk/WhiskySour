...
- Clone the repo, then run `scripts/build-and-run.sh`. It builds `WhiskeySour.app` using `xcodebuild`, copies the bundle to the repo root, and immediately launches it so the installer can download `Libraries.tar.gz` (attached to the `v3.1.1` release) automatically into `~/Library/Application Support/com.jhmk.WhiskySour/Libraries/`.
